...
class TMDB{
	 ...
	 *	@deprecated
	 */
	public function getMovieInfo($idMovie, $option = '', $append_request = '', $language = true){
		$option = (empty($option)) ? '' : '/' . $option;
		$params = 'movie/' . $idMovie . $option;
		$result = $this->_call($params, $append_request, $language);
	 ...
	 * 	@return string
	 */
	public function _call($action, $appendToResponse, $language = true)
	{

		$url = self::_API_URL_ . $action . '?api_key=' . $this->getApikey();
		if ($language) {
			$url .= '&language=' . $this->getLang();
		}
	 ...
	 * 	Get a Genre
	 *
	 * 	@param int $idGenre The Genre id
	 * 	@return Genre
	 */
	public function getGenre($idGenre){
		return new Genre($this->_call('genre/' . $idGenre, ''));
	}

	/**
	 * 	Get a Company
	 *
	 * 	@param int $idCompany The Genre id
	 * 	@return Company
	 */
	public function getCompany($idCompany){
		return new Company($this->_call('company/' . $idCompany, ''));
	}
