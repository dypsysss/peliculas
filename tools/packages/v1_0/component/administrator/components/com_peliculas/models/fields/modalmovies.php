 ...
 */
class JFormFieldModalMovies extends JFormField
{
    /**
     * The form field type.
     */
    protected $type = 'ModalMovies';
    ...
    protected function getInput()
    {
        ...
        $link = 'index.php?option=com_peliculas&amp;view=movies&amp;layout=modal&amp;tmpl=component&amp;field=' . $this->id . '&amp;emovieid=-1';
        ...
        // Build the script.
        $script = array();
        $script[] = 'function jModalMovies_' . $this->id . '(eMovieId, id, title) {';
        ...
        $dspItem = '';
        if ($this->value) {
            $tblItem = JTable::getInstance('Movies', 'PeliculasTable');
            $tblItem->load($this->value);
            $dspItem = $tblItem->name;
        }

        $html[] = '<input type="text" id="dsp_' . $this->id . '" value="' . $dspItem . '"' . ' readonly' . $attr . ' />';
