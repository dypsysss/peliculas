 ...
 */
class PeliculasModelEvents extends PeliculasModelBaseList
{
    /**
     * Constructor.
     *
    ...
    protected function getListQuery()
    {
        ...
        // Join over the cinemas.
        $query->select('ci.name AS cinema_name')
            ->join('LEFT', '#__peliculas_cinemas AS ci ON ci.id = a.cinema_id');
        ...
        // Filter by client.
        $cinemaId = $this->getState('filter.cinema_id');

        if (is_numeric($cinemaId))
        {
            $query->where('a.cinema_id = ' . (int) $cinemaId);
        }
        ...
        if (strtolower($orderCol) == 'cinema_name') {
            $orderCol = 'ci.name';
        }
